...
const ProgressBar = ({TrackingType, Status, DocumentType, ClaimType, Mode}) => {
  ...
  if (TrackingType === 'PY') {
    if (ClaimType.toUpperCase() === 'CHECK') {
        if (DocumentType && DocumentType.toUpperCase() !== 'BOND - RETENTION MONEY') {
            statusScheme = [
                '1* Encoded',
            ...
            if (Status === 'Forwarded to Admin - Operation') {
              statusScheme.push('12*Forwarded to Admin - Operation', '13*Check Advised');
            } else if (Status === 'Forwarded to Admin - Administration') {
              statusScheme.push('12*Forwarded to Admin - Administration', '13*Check Advised');
        ...
        } else if (DocumentType && DocumentType.toUpperCase() === 'BOND - RETENTION MONEY') {
            statusScheme = [
                '1* Encoded',
                '2* CAO Received',
                'x* Pending at CAO',
   ...
   if (TrackingType === 'PX' || TrackingType === 'Payment') {
    ...
    if (Status === 'Forwarded to Admin - Operation') {
      statusScheme.push('10*Forwarded to Admin - Operation', '11*Check Advised');
    } else if (Status === 'Forwarded to Admin - Administration') {
      statusScheme.push('10*Forwarded to Admin - Administration', '11*Check Advised');
    } else if (Status === 'Forwarded to SP - Admin') {
