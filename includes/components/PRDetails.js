...
import { GLOBAL_STYLES as styles, COLORS, SPACING, FONT_SIZES } from './../styles/theme'; // Ensure path is correct
...
export const GeneralInformationCard = ({ genInformationData }) => (
  <View style={styles.sectionContainer}> 
    ...
    {[
      ...
      { label: 'OBR Number', value: genInformationData.OBR_Number, icon: 'document-text-outline' },
      { label: 'PR Number', value: genInformationData.PR_Number, icon: 'receipt-outline' },
      { label: 'PR Sched', value: genInformationData.PR_Sched, icon: 'calendar-outline' },
      { label: 'Fund', value: genInformationData.Fund, icon: 'cash-outline' },
      { label: 'Encoded By', value: genInformationData.EncodedBy, icon: 'person-outline' },
      {
        label: 'Date Encoded',
...
export const OBRInformationCard = ({ OBRInformation, totalAmount, insertCommas }) => (
  <View style={styles.sectionContainer}>
    <View style={styles.sectionHeader}>
    ...
    <View style={styles.sectionTable}> 
      <View style={styles.tableHeader}>
        ...
        <Text style={[styles.tableHeaderText, { flex: 1, textAlign: 'center' }]}>CODE</Text>
      ...
      {OBRInformation && OBRInformation.length > 0 ? (
        OBRInformation.map((item, index, arr) => (
          ...
          >
            <View style={{ flex: 1 }}>
              <Text style={[styles.tableRowMainText, { paddingVertical: SPACING.xs }]}>{item.PR_ProgramCode}</Text>
              <Text style={styles.tableRowSubText}>{item.ProgramName}</Text>
            </View>
            <View style={{ flex: 1, alignItems: 'center' }}>
              <Text style={[styles.tableRowMainText, { paddingVertical: SPACING.xs }]}>{item.PR_AccountCode}</Text>
              <Text style={styles.tableRowSubText}>{item.AccountTitle}</Text>
            </View>
            <View style={{ flex: 1, alignItems: 'flex-end' }}>
              <Text style={[styles.tableRowMainText, { paddingVertical: SPACING.xs }]}>
...
export const PRDetailsCard = ({ prpopxDetails, prpopxTotalAmount, insertCommas, expandedIndex, toggleDescription }) => (
  <View style={styles.sectionContainer}> 
    <View style={styles.sectionHeader}> 
          ...
          <View key={index}>
            <View style={[
              ...
              <View style={{ flex: 2, paddingRight: SPACING.s }}>
                <TouchableOpacity onPress={() => toggleDescription(index)} style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <Text
...
export const DigitalCopiesCard = ({
  ...
  <View style={styles.sectionContainer}> 
    ...
    <View style={styles.digitalCopiesContent}>
      {(formTypeMap?.PR || []).length > 0 ? (formTypeMap.PR.map((formType, index, arr) => {
        const formTypeFiles =
          ...
          >
            <View style={styles.digitalCopyItemHeader}>
              ...
              <TouchableOpacity
                ...
                <Icon name={'add-circle-outline'} size={FONT_SIZES.large} color={COLORS.white} />
                <Text style={[styles.buttonSmallText, styles.attachButtonText, { marginLeft: SPACING.xs }]}>
                  Attach
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                <Icon name={'trash-outline'} size={FONT_SIZES.large} color={COLORS.deleteIconColor} />
              </TouchableOpacity>
            </View>
            <View style={{ marginTop: SPACING.xs }}>
              {hasFiles ? (
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (fileExtension === 'pdf') {
                            Linking.openURL(uniqueUri).catch(err => console.error("Failed to open PDF", err));
                          } else {
                            handleImagePress(uniqueUri);
                          }
                        }}>
                        <Icon
                          name={fileExtension === 'pdf' ? 'document-text-outline' : 'image-outline'}
                          size={FONT_SIZES.medium}
                          color={COLORS.primary}
                        />
                        <Text style={styles.fileEntryText} numberOfLines={1} ellipsizeMode="middle">
                          {fileName}
                        </Text>
                        <Icon
                          name={'eye-outline'}
                ...
                <Text style={[styles.noDataText, { paddingVertical: SPACING.s, textAlign: 'left' }]}>
                  No attached files
                </Text>
              )}
            </View>
...
export const TransactionHistoryCard = ({ transactionHistory, removeHtmlTags }) => (
  <View style={styles.sectionContainer}> 
    <View style={styles.sectionHeader}> 
      ...
      {transactionHistory && transactionHistory.length > 0 ? (
        transactionHistory.map((item, index, arr) => (
          <View key={index}>
            <View
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.transactionIndexText}>
                 {/*  <Icon name={'ellipse'} size={FONT_SIZES.small} color={COLORS.accent} /> */} {index + 1}
                </Text>
              </View>
              <View style={{ flex: 3 }}>
            ...
            {index !== arr.length - 1 && (
              <View style={[styles.divider, { marginHorizontal: 0, marginTop: 0 }]} />
            )}
          </View>
        ))
      ) : (
