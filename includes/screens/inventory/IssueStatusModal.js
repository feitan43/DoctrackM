...
const IssueStatusModal = ({ visible, status, message, onClose }) => {
  const isSuccess = status === 'success';
  const iconName = isSuccess ? 'checkmark-circle' : 'close-circle';
  const headerColors = isSuccess ? ['#28a745', '#218838'] : ['#dc3545', '#c82333']; // Green for success, Red for error
  const titleText = isSuccess ? 'Issue Successful!' : 'Issue Failed!';
